...
const WhatweOfferMobile = ({id, item}:any) => {
  ...
  return (
    <>
    <section ref={ref} className={`relative z-[22] p-4 min-h-[550px]  min-w-full flex flex-col transition-all duration-800 ease-in-out justify-center items-center wwo${id}`}>
      <div className="relative bg-[url('/images/what-we-offer-bg.png')] container mx-auto rounded-[32px] bg-left-bottom bg-no-repeat bg-cover grid md:h-[506px] grid-cols-1 md:grid-cols-2">
        <div className="flex justify-center items-start max-w-2xl flex-col p-6 md:p-10 lg:p-32">
          ...
          <h5 className="text-2xl md:text-[32px] font-bold text-white mb-4">{item.title}</h5>
        ...
        <ul className="gap-2 flex md:bottom-1/2  right-1/2 md:right-4 -bottom-6 lg:right-10 absolute md:flex-col translate-x-1/2 transform md:translate-y-1/2 px-4 z-50">
          {
            [1,2,3,4].map((dots,idx)=>{
              return(
                <li key={idx} className={`bg-[#457575]  p-1 rounded-full ${id === idx && 'md:py-2 px-2 md:px-1 md:bg-white bg-[#042c2cc0]'}`}></li>
              )
            })
          }
        </ul>
